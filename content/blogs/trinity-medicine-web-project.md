---
title: Trinity Medicine Web Project
date: 6th July 2022
description: Aligning with Trinity Strategic Plan 2020-2025, I worked on a large-scale web redevelopment and migration project from 2021 to 2022, collaborating with the IT department at Trinity College Dublin..
image: /blogs-img/web-project-in-trinity-feature-image.png
alt: Trinity Biomedical Sciences Institute (TBSI), Trinity College Dublin
ogImage: /blogs-img/web-project-in-trinity-feature-image.png
tags: ['accessibility', 'WCAG', 'Siteimprove', 'School of Medicine', 'Trinity College Dublin']
published: true
---

**Role:** Web Admin and Digital Accessibility Specialist

### Overview

Aligning with the Trinity Strategic Plan 2020–2025, I worked on a large-scale web redevelopment and migration project between 2021 and 2022, collaborating with the [IT department](https://www.tcd.ie/itservices/) at Trinity College Dublin.

### Context

![The landing page of the old SoM site](blogs-img/old-som-landing-page.png)

The landing page of the old SoM site

![The landing page of the new SoM site](blogs-img/new-som-landing-page.png)

The landing page of the new SoM site

The School of Medicine website, https://www.tcd.ie/medicine/, was one of the university’s largest and most complex information-rich web areas, comprising a network of around 700 microsites across more than 40 disciplines at the time of the project. As a key member of the School of Medicine team, I worked closely with colleagues across the school, used Trello to map out project activities, and provided training and technical support throughout the process.

Building on Trinity Theme 2015, the university’s new public website used a different platform and hosting environment, with a more modern design and user interface. My work covered sitemap restructuring, web inventory tracking, media asset cataloguing, redesign of page layouts, the recreation and migration of large volumes of digital content, and technical troubleshooting. I also supported the clean-up of legacy content by standardising file names, identifying outdated files and broken links, and repairing corrupted images and documents.

I also used Siteimprove to review and improve site quality, applying best practices in search engine optimisation, metadata, accessibility, W3C standards and user experience. These improvements contributed to the overall site performance and supported the successful launch of the new School of Medicine website in early 2023.

### Project Links

- [SoM site](https://www.tcd.ie/medicine/)